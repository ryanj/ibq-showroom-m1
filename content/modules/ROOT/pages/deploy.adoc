...
%prod.quarkus.openshift.route.target-port=http <6>
%prod.quarkus.openshift.route.tls.termination=edge <7>
%prod.quarkus.openshift.route.tls.insecure-edge-termination-policy=Allow <8>
quarkus.openshift.deployment-kind=Deployment <9>
quarkus.container-image.group=kubeadmin-project <10>
quarkus.container-image.registry=image-registry.openshift-image-registry.svc:5000 <11>
----
...
<6> Specify the target port
<7> Spefify the termination type
<8> Spefify the desired behavior for insecure connections to a route
<9> Generate the Deployment resource
<10> Specify a project where the application is deployed
<11> Sepcify an internal container registry to push an application image
